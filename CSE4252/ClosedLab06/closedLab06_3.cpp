...
int main()
{
  ...
  std::map<string,Drone> mapOfDrones;
  string Name;
  string quad;
  int id; 
	...
	for(int i = 0; i <= 4; i++){
		...
		cout << "Enter id:" << endl;
		cin >> id;
		cout << "Enter quadrant" << endl;
		cin >> quad;
		
		Drone *drn;
		drn = new Drone(Name,id);
		//mapOfDrones.insert(std::pair<string,Drone>(quad,*drn));
		mapOfDrones[quad] = *drn;
		cin.get();
	}
	
	std::map<string,Drone>::iterator it;
  for ( it = mapOfDrones.begin(); it != mapOfDrones.end(); it++ ) {
    // For each Drone, print out their info
    cout << it->first << ": " ;
    it->second.displayDrone();
  }
