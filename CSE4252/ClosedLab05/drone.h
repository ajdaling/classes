...
public:
  ...
  void displayDrone();
  static int getNextDroneSequenceNumber();             // this function returns a next sequence number for a new Drone
  void update_sensor_values(float r, float p, float y, int d, float v);
  static int droneSequenceNumber;                     // the drone sequence number variable controls the drone ID assignment
