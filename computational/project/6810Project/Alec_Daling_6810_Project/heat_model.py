...
class heat_model(object):
	...
	def time_step(self):
		'''
		creates timer and invokes relevant algorithm for dense models
		'''
		#update u
		if self.is_sparse:
...
# 				self.bd_1d()
	...
	def time_step_sparse(self):
		'''
		creates timer and invoked relevant iteration algorithm for sparse models
		'''
		if self.method == 'fd':
# 			self.fd_sparse()
			if not self.is2d:
...
# 			self.bd_1d_sparse()
	...
	def fd_1d(self):
		'''
		performs iteration for dense forward difference algorithm in 1-D
		'''
		for i in range(1,self.lx):
	...
	def fd_2d(self):
		'''
		performs iteration for dense forward difference algorithm in 2-D
		'''
		for i in range(1,self.lx):
			for j in range(1,self.ly):
	...
	def fd_sparse_1d(self):
		'''
		performs iteration for sparse forward difference algorithm in 1-D
		'''
	...
	def fd_sparse_2d(self):
		'''
		performs iteration for sparse forward difference algorithm in 2d
		'''
		N = len(self.u[0])
	...
	def bd_1d(self):
		'''
		performs iteration for dense backward difference algorithm in 1-D
		'''
		l = len(self.u)
		A = zeros((l,l))
		b = zeros(l)
		...
		print(b)
 		self.u[:] = solve(A,b)
		self.u[0] = self.bc_xmin
		self.u[-1] = self.bc_xmax
		self.u_1[:] = self.u

	def bd_1d_sparse(self):
		'''
		performs iteration for sparse backward difference algorithm in 1-D
		'''
...
# 		A = self.get_tridiagonal()
	...
	def resize(self):
		'''
		updates all relevant parameters when dimension slider is changed 
		'''
		#reset dimensions
		if not self.is2d:
			print("resizing 1d")
	...
	def update_bc(self):
		'''
		updates boundary conditions when user changed slider
		'''
		if not self.is2d:
